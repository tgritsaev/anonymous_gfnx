"""Single-file implementation for Detailed Balance on GFP environment.

Run the script with the following command:
```bash
python baselines/db_gfp.py
```
...
def train_step(idx: int, train_state: TrainState) -> TrainState:
    ...
    def fwd_policy_fn(rng_key: chex.PRNGKey, env_obs: gfnx.TObs, policy_params) -> chex.Array:
        ...
        # With probability cur_epsilon, return zero logits and the same policy outputs
        do_explore = jax.random.bernoulli(explore_key, cur_epsilon, shape=(batch_size,))
...
@hydra.main(config_path="configs/", config_name="db_gfp", version_base=None)
def run_experiment(cfg: OmegaConf) -> None:
    ...
    def gfp_distance_fn(lhs_state: gfnx.GFPEnvState, rhs_state: gfnx.GFPEnvState) -> chex.Array:
        """Compute the distance between two GFP states."""
        return gfnx.utils.distances.hamming_distance(lhs_state.tokens, rhs_state.tokens, env.nchar)
