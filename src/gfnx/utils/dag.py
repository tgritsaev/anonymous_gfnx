...
def get_transitive_closure(adjacency_matrix: chex.Array) -> chex.Array:
    ...
    def power_iteration(i: int, carry: tuple[chex.Array, chex.Array]) -> tuple[chex.Array, chex.Array]:
    ...
    return jax.lax.fori_loop(0, num_variables, power_iteration, (adjacency_matrix, adjacency_matrix))[1]
...
def get_markov_blanket(adjacency_matrix: chex.Array) -> chex.Array:
    ...
    num_variables = adjacency_matrix.shape[0]
    blanket = jax.vmap(get_single_blanket, in_axes=(None, 0))(adjacency_matrix, jnp.arange(num_variables))
...
def generate_dataset(
    ...
    # Sample train data
    node_names, train_data = sample_from_linear_gaussian(graph, num_samples=num_train_samples, rng=data_rng)
